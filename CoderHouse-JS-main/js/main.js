...
const btnPersonaje = document.getElementById("crearJugador").addEventListener("click", function(){        
    ...
    if(nombre !== ""){
      ...
      const jugador2 = creacionPersonaje2();
      agregarItem2(jugador2);
        console.log("jugador 1 creado:", jugador1);
        input.value = "";
        agregarItem(jugador1);
        console.log("Jugador 1:", jugador1);
        document.getElementById("infoJugador1").innerHTML = `
        <strong>Nombre:</strong> ${jugador1.nombre} <br>
        <strong>Vida:</strong> ${jugador1.vida}/${jugador1.vidaMaxima} <br>
        <strong>Ataque:</strong> ${jugador1.ataque} <br>
        <strong>Defensa:</strong> ${jugador1.defensa} <br>
        <strong>Velocidad:</strong> ${jugador1.velocidad} <br>
        <strong>Items:</strong> 
        <ul>
          ${jugador1.items.map(item => `<li>${item.nombre} (ATQ:${item.ataque}, DEF:${item.defensa}, VEL:${item.velocidad})</li>`).join("")}
        </ul>
        `;
      } else {
  ...
  log.push(`Pelea entre ${j1.nombre} y ${j2.nombre}`);
    ...
    while (j1.vida > 0 && j2.vida >0){    
      ...
      if (itemEquipado && Math.random() < 0.2){
          ...
          if (esquiva) {
            log.push(`${rival.nombre} esquiva el ataque de ${turno.nombre}`);
